...
const EditVenueScreen = ({ route, navigation }) => {
    ...
    useEffect(() => {
        const fetchVenue = async () => {
            try {
                ...
                setName(data.name); setDescription(data.description); setAddress(data.address); setCity(data.city); setState(data.state);
                setPricePerHour(data.pricePerHour.toString()); setSportTypes(data.sportTypes.join(', ')); setImage(data.images[0]);
                setAmenities(prev => ({ ...prev, ...data.amenities }));
            } catch (error) { Alert.alert('Error', 'Could not fetch venue details'); navigation.goBack(); } finally { setLoading(false); }
        };
        fetchVenue();
    }, [venueId]);
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            formData.append('sportTypes', JSON.stringify(sportsArray)); formData.append('amenities', JSON.stringify(amenities));
            if (newImage) { const filename = newImage.split('/').pop(); const match = /\.(\w+)$/.exec(filename); const type = match ? `image/${match[1]}` : `image`; formData.append('images', { uri: newImage, name: filename, type }); }
            await api.put(`/venues/${venueId}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
            Alert.alert('Success', 'Venue Updated Successfully!', [{ text: 'OK', onPress: () => navigation.goBack() }]);
        } catch (error) { Alert.alert('Error', error.response?.data?.message || 'Could not update venue'); } finally { setUpdating(false); }
    };

    const toggleAmenity = (key) => setAmenities(prev => ({ ...prev, [key]: !prev[key] }));

    if (loading) return <ActivityIndicator size="large" style={{ flex: 1, justifyContent: 'center', backgroundColor: colors.background }} color={colors.accent} />;
        ...
        <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 120 }}>
            <View style={styles.imageSection}>
                <TouchableOpacity onPress={pickImage} style={styles.imagePicker}>
                    ...
                    <View style={styles.overlay}><Ionicons name="camera" size={24} color="#fff" /><Text style={{ color: '#fff', marginLeft: 5 }}>Change Image</Text></View>
                </TouchableOpacity>
            </View>
            <View style={styles.form}>
                ...
                <TextInput style={[styles.input, styles.textArea]} value={description} onChangeText={setDescription} multiline numberOfLines={3} placeholderTextColor={colors.textSecondary} />
                <Text style={styles.label}>Address</Text>
                <TextInput style={styles.input} value={address} onChangeText={setAddress} placeholderTextColor={colors.textSecondary} />
                <View style={styles.row}>
                    <View style={{ flex: 1, marginRight: 10 }}><Text style={styles.label}>City</Text><TextInput style={styles.input} value={city} onChangeText={setCity} placeholderTextColor={colors.textSecondary} /></View>
                    <View style={{ flex: 1 }}><Text style={styles.label}>State</Text><TextInput style={styles.input} value={state} onChangeText={setState} placeholderTextColor={colors.textSecondary} /></View>
                </View>
                <Text style={styles.label}>Price/Hour (₹)</Text>
                <TextInput style={styles.input} value={pricePerHour} onChangeText={setPricePerHour} keyboardType="numeric" placeholderTextColor={colors.textSecondary} />
                <Text style={styles.label}>Sports</Text>
                <TextInput style={styles.input} value={sportTypes} onChangeText={setSportTypes} placeholderTextColor={colors.textSecondary} />
