...
const EditSportsProfileScreen = ({ navigation, route }) => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      setProfile(res.data);
      setSportName(res.data.sportName);
      setSkillLevel(res.data.skillLevel);
      setPlaystyle(res.data.playstyle);
      setExperienceMonths(res.data.experienceMonths.toString());
      setPosition(res.data.position || '');
      setDominantSide(res.data.dominantSide || '');
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      await api.put(`/sports-profiles/${profileId}`, {
      ...
      console.log('Error updating sports profile:', err);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>Edit Sports Profile</Text>
        <View style={{ width: 24 }} />
      </View>
...
export default EditSportsProfileScreen;
